...
(function () {
    ...
    function ResultsController($scope, $http, $filter, $state, testapiApiUrl, raiseAlert, ngDialog, $resource) {
        ...
        if (!ctrl.isUserResults && !ctrl.isReviewer && !ctrl.isAdministrator) {
            $state.go('home');
        }
        ...
        function submitApplication(result) {
            ...
            var data = {
                ...
                "certification_type": ctrl.cert_type,
        ...
        function toApprove(result, value) {
            var resp = confirm('Once you approve a test result, user will be able to submit an Application. Do you want to proceed?');
            if (resp) {
