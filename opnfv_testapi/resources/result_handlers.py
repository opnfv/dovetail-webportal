...
class ResultsUploadHandler(ResultsCLHandler):
    ...
    def post(self):
        ...
        try:
            ...
            if not 'results/results.json' in tar_files:
        ...
        try:
            ...
            msg = 'Uploaded results don''t appear to contain a valid results.json file!'
            self.finish_request({'code': 403, 'msg': msg})
            return
        # Build a test id from the build_tag, where this field takes a couple of different
        # formats between Dovetail NFVI testing and VNF testing. If a valid UUID isn't 
        # part of the build_tag, we will generate one here.
        test_id_match = re.search('[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}', build_tag)
        if not test_id_match:
            print('test_id doesn''t look to be a valid UUID, generating a new one...')
